...
import {CreateProduct, UpdateProduct} from "../../../../DB/interfaces/product.interface"
export const createProduct = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
 ...
 export const updateProduct = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
   ...
   const updatedProduct = await productModel.findOneAndUpdate({_id:id},newProduct,{new:true});
    if (updatedProduct) {
 ...
 export const products = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
...
export const product = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
   const  {id} = req.params
   const product = await productModel.findById(id);
   if (!product) {
    const error = new Error("can not find product");
